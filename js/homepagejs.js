  ...
  $(function() {
	...
	$('#fromtimepicker').timepicker({ 'timeFormat': 'H:i:s',step: 15 });
	$('#totimepicker').timepicker({ 'timeFormat': 'H:i:s',step: 15 });
  });
  
  
 

function showAvailability(floorno,str,username)
{
//alert(username);
//var username = document.getElementById("username").value;
document.getElementById("donespace").innerHTML="";
...
xmlhttp.open("GET","getAvailability.php?inputDate="+str+"&floorno="+floorno+"&un="+username,true);
xmlhttp.send();
//document.write("hi");
}
...
function deleteRow(r,keyToDelete)
{
//document.getElementById("deleteSpace").innerHTML = "bleh beleh hee";
//alert(keyToDelete);
if(window.XMLHttpRequest){
	xmlhttp = new XMLHttpRequest();
	}
...
xmlhttp.onreadystatechange = function(){
	if(xmlhttp.readyState == 4 && xmlhttp.status == 200){
		document.getElementById("deleteSpace").innerHTML = xmlhttp.responseText;
	}
}
xmlhttp.open("GET","deleteBooking.php?key="+keyToDelete,true);
xmlhttp.send();
